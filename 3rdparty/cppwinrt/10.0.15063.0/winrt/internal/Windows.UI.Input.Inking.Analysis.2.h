// C++ for the Windows Runtime v1.0.170331.7
// Copyright (c) 2017 Microsoft Corporation. All rights reserved.

#pragma once

#include "Windows.UI.Input.Inking.Analysis.1.h"

WINRT_EXPORT namespace winrt {

namespace ABI::Windows::Foundation::Collections {

#ifndef WINRT_GENERIC_e2fcc7c1_3bfc_5a0b_b2b0_72e769d1cb7e
#define WINRT_GENERIC_e2fcc7c1_3bfc_5a0b_b2b0_72e769d1cb7e
template <> struct __declspec(uuid("e2fcc7c1-3bfc-5a0b-b2b0-72e769d1cb7e")) __declspec(novtable) IIterable<hstring> : impl_IIterable<hstring> {};
#endif

#ifndef WINRT_GENERIC_2f13c006_a03a_5f69_b090_75a43e33423e
#define WINRT_GENERIC_2f13c006_a03a_5f69_b090_75a43e33423e
template <> struct __declspec(uuid("2f13c006-a03a-5f69-b090-75a43e33423e")) __declspec(novtable) IVectorView<hstring> : impl_IVectorView<hstring> {};
#endif

#ifndef WINRT_GENERIC_98b9acc1_4b56_532e_ac73_03d5291cca90
#define WINRT_GENERIC_98b9acc1_4b56_532e_ac73_03d5291cca90
template <> struct __declspec(uuid("98b9acc1-4b56-532e-ac73-03d5291cca90")) __declspec(novtable) IVector<hstring> : impl_IVector<hstring> {};
#endif

#ifndef WINRT_GENERIC_e5ce1a07_8d33_5007_ba64_7d2508ccf85c
#define WINRT_GENERIC_e5ce1a07_8d33_5007_ba64_7d2508ccf85c
template <> struct __declspec(uuid("e5ce1a07-8d33-5007-ba64-7d2508ccf85c")) __declspec(novtable) IVectorView<uint32_t> : impl_IVectorView<uint32_t> {};
#endif

#ifndef WINRT_GENERIC_421d4b91_b13b_5f37_ae54_b5249bd80539
#define WINRT_GENERIC_421d4b91_b13b_5f37_ae54_b5249bd80539
template <> struct __declspec(uuid("421d4b91-b13b-5f37-ae54-b5249bd80539")) __declspec(novtable) IIterable<uint32_t> : impl_IIterable<uint32_t> {};
#endif

#ifndef WINRT_GENERIC_bbc11401_89d0_5305_a3b3_36c887714b9b
#define WINRT_GENERIC_bbc11401_89d0_5305_a3b3_36c887714b9b
template <> struct __declspec(uuid("bbc11401-89d0-5305-a3b3-36c887714b9b")) __declspec(novtable) IIterable<Windows::UI::Input::Inking::InkStroke> : impl_IIterable<Windows::UI::Input::Inking::InkStroke> {};
#endif


}

namespace ABI::Windows::Foundation {

#ifndef WINRT_GENERIC_2c46d1bd_6dbb_5007_ba85_3d0106bddf50
#define WINRT_GENERIC_2c46d1bd_6dbb_5007_ba85_3d0106bddf50
template <> struct __declspec(uuid("2c46d1bd-6dbb-5007-ba85-3d0106bddf50")) __declspec(novtable) IAsyncOperation<Windows::UI::Input::Inking::Analysis::InkAnalysisResult> : impl_IAsyncOperation<Windows::UI::Input::Inking::Analysis::InkAnalysisResult> {};
#endif


}

namespace ABI::Windows::Foundation::Collections {

#ifndef WINRT_GENERIC_0b7b4c9d_182f_582a_bddb_42b1aac30cad
#define WINRT_GENERIC_0b7b4c9d_182f_582a_bddb_42b1aac30cad
template <> struct __declspec(uuid("0b7b4c9d-182f-582a-bddb-42b1aac30cad")) __declspec(novtable) IVectorView<Windows::Foundation::Point> : impl_IVectorView<Windows::Foundation::Point> {};
#endif

#ifndef WINRT_GENERIC_2b3fee11_53b5_55b0_8d71_c40b427de029
#define WINRT_GENERIC_2b3fee11_53b5_55b0_8d71_c40b427de029
template <> struct __declspec(uuid("2b3fee11-53b5-55b0-8d71-c40b427de029")) __declspec(novtable) IVectorView<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> : impl_IVectorView<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> {};
#endif

#ifndef WINRT_GENERIC_6744f458_b242_5767_a643_996e01dff0e4
#define WINRT_GENERIC_6744f458_b242_5767_a643_996e01dff0e4
template <> struct __declspec(uuid("6744f458-b242-5767-a643-996e01dff0e4")) __declspec(novtable) IVectorView<Windows::UI::Input::Inking::InkStroke> : impl_IVectorView<Windows::UI::Input::Inking::InkStroke> {};
#endif

#ifndef WINRT_GENERIC_c192280d_3a09_5423_9dc5_67b83ebde41d
#define WINRT_GENERIC_c192280d_3a09_5423_9dc5_67b83ebde41d
template <> struct __declspec(uuid("c192280d-3a09-5423-9dc5-67b83ebde41d")) __declspec(novtable) IIterable<Windows::Foundation::Point> : impl_IIterable<Windows::Foundation::Point> {};
#endif

#ifndef WINRT_GENERIC_c0d513a9_ec4a_5a5d_b6d5_b707defdb9f7
#define WINRT_GENERIC_c0d513a9_ec4a_5a5d_b6d5_b707defdb9f7
template <> struct __declspec(uuid("c0d513a9-ec4a-5a5d-b6d5-b707defdb9f7")) __declspec(novtable) IVector<Windows::Foundation::Point> : impl_IVector<Windows::Foundation::Point> {};
#endif

#ifndef WINRT_GENERIC_534832ed_2a03_5604_890d_5a928cd427b9
#define WINRT_GENERIC_534832ed_2a03_5604_890d_5a928cd427b9
template <> struct __declspec(uuid("534832ed-2a03-5604-890d-5a928cd427b9")) __declspec(novtable) IVector<uint32_t> : impl_IVector<uint32_t> {};
#endif

#ifndef WINRT_GENERIC_8c304ebb_6615_50a4_8829_879ecd443236
#define WINRT_GENERIC_8c304ebb_6615_50a4_8829_879ecd443236
template <> struct __declspec(uuid("8c304ebb-6615-50a4-8829-879ecd443236")) __declspec(novtable) IIterator<hstring> : impl_IIterator<hstring> {};
#endif

#ifndef WINRT_GENERIC_f06a2739_9443_5ef0_b284_dc5aff3e7d10
#define WINRT_GENERIC_f06a2739_9443_5ef0_b284_dc5aff3e7d10
template <> struct __declspec(uuid("f06a2739-9443-5ef0-b284-dc5aff3e7d10")) __declspec(novtable) IIterator<uint32_t> : impl_IIterator<uint32_t> {};
#endif

#ifndef WINRT_GENERIC_2640111c_bc3f_564b_b931_b7a378933f58
#define WINRT_GENERIC_2640111c_bc3f_564b_b931_b7a378933f58
template <> struct __declspec(uuid("2640111c-bc3f-564b-b931-b7a378933f58")) __declspec(novtable) IVector<Windows::UI::Input::Inking::InkStroke> : impl_IVector<Windows::UI::Input::Inking::InkStroke> {};
#endif

#ifndef WINRT_GENERIC_5608d5a9_e7e4_5a0b_941f_b7fed76b35bf
#define WINRT_GENERIC_5608d5a9_e7e4_5a0b_941f_b7fed76b35bf
template <> struct __declspec(uuid("5608d5a9-e7e4-5a0b-941f-b7fed76b35bf")) __declspec(novtable) IIterator<Windows::UI::Input::Inking::InkStroke> : impl_IIterator<Windows::UI::Input::Inking::InkStroke> {};
#endif


}

namespace ABI::Windows::Foundation {

#ifndef WINRT_GENERIC_a7ef2666_6fc4_568f_bbf3_19c1036a26bf
#define WINRT_GENERIC_a7ef2666_6fc4_568f_bbf3_19c1036a26bf
template <> struct __declspec(uuid("a7ef2666-6fc4-568f-bbf3-19c1036a26bf")) __declspec(novtable) AsyncOperationCompletedHandler<Windows::UI::Input::Inking::Analysis::InkAnalysisResult> : impl_AsyncOperationCompletedHandler<Windows::UI::Input::Inking::Analysis::InkAnalysisResult> {};
#endif


}

namespace ABI::Windows::Foundation::Collections {

#ifndef WINRT_GENERIC_c602b59e_0a8e_5e99_b478_2b564585278d
#define WINRT_GENERIC_c602b59e_0a8e_5e99_b478_2b564585278d
template <> struct __declspec(uuid("c602b59e-0a8e-5e99-b478-2b564585278d")) __declspec(novtable) IIterator<Windows::Foundation::Point> : impl_IIterator<Windows::Foundation::Point> {};
#endif

#ifndef WINRT_GENERIC_cceb201d_686f_5039_9ec9_db10b8390179
#define WINRT_GENERIC_cceb201d_686f_5039_9ec9_db10b8390179
template <> struct __declspec(uuid("cceb201d-686f-5039-9ec9-db10b8390179")) __declspec(novtable) IVector<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> : impl_IVector<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> {};
#endif

#ifndef WINRT_GENERIC_ad35ed5c_5f8c_5a68_a6e1_67f209a05ea7
#define WINRT_GENERIC_ad35ed5c_5f8c_5a68_a6e1_67f209a05ea7
template <> struct __declspec(uuid("ad35ed5c-5f8c-5a68-a6e1-67f209a05ea7")) __declspec(novtable) IIterator<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> : impl_IIterator<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> {};
#endif

#ifndef WINRT_GENERIC_784f069e_badd_5258_bd8f_42ce205cc95a
#define WINRT_GENERIC_784f069e_badd_5258_bd8f_42ce205cc95a
template <> struct __declspec(uuid("784f069e-badd-5258-bd8f-42ce205cc95a")) __declspec(novtable) IIterable<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> : impl_IIterable<Windows::UI::Input::Inking::Analysis::IInkAnalysisNode> {};
#endif


}

namespace Windows::UI::Input::Inking::Analysis {

struct IInkAnalysisInkBullet :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisInkBullet>,
    impl::require<IInkAnalysisInkBullet, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisInkBullet(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisInkDrawing :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisInkDrawing>,
    impl::require<IInkAnalysisInkDrawing, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisInkDrawing(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisInkWord :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisInkWord>,
    impl::require<IInkAnalysisInkWord, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisInkWord(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisLine :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisLine>,
    impl::require<IInkAnalysisLine, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisLine(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisListItem :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisListItem>,
    impl::require<IInkAnalysisListItem, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisListItem(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisNode :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisNode>
{
    IInkAnalysisNode(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisParagraph :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisParagraph>,
    impl::require<IInkAnalysisParagraph, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisParagraph(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisResult :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisResult>
{
    IInkAnalysisResult(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisRoot :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisRoot>,
    impl::require<IInkAnalysisRoot, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisRoot(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalysisWritingRegion :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalysisWritingRegion>,
    impl::require<IInkAnalysisWritingRegion, Windows::UI::Input::Inking::Analysis::IInkAnalysisNode>
{
    IInkAnalysisWritingRegion(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalyzer :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalyzer>
{
    IInkAnalyzer(std::nullptr_t = nullptr) noexcept {}
};

struct IInkAnalyzerFactory :
    Windows::Foundation::IInspectable,
    impl::consume<IInkAnalyzerFactory>
{
    IInkAnalyzerFactory(std::nullptr_t = nullptr) noexcept {}
};

}

}
